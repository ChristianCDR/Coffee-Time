...
export default function Navbar() {
    ...
    return (
        ...
        >
            ...
            <div className="lg:hidden" onClick={toggleMenu} aria-expanded={open ? "true" : "false"} aria-controls="mobile-menu">
            ...
            <div className="hidden lg:block w-1/4 block">
                <MenuLinks className = "flex flex-row justify-around items-left p-4 text-sm font-bold"/>
            </div>
            ...
            <div className={`fixed z-1 right-0 top-23 h-full w-full bg-white transform transition-all duration-800 ease-in-out ${ open ? 'translate-x-0' : 'translate-x-full' }`}>
                <MenuLinks className = "flex flex-col justify-around h-1/3 text-sm text-black font-bold"/>
            </div>
        </div>
    );
}